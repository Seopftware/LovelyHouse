...
<a href='view.php?bno=<?php echo $bNo; ?>&re_wt=Y&d_no=<?php echo $d['no'];?>&#lo_reply_2' onfocus="this.blur()">
...
<td colspan='4' valign='top'bgcolor='#E3E0E0'>
		...
		<div align='right'>
		<!-- <a href='view.php?bno=<?php echo $bNo; ?>&re_wt=Y&d_no=<?php echo $d['no'];?>&#lo_reply_2' onfocus="this.blur()"> -->
		<a href='comment_del.php?d_no=<?=$d['no']?>&no_s=<?php echo $bNo;?>&bbs1_no=<?=$d['bbs1_no']?>&replys=<?php echo $d['replys'];?>	&replys_all=all' onfocus='this.blur()'>
		<font color='#FF0000' onclick="return confirm('정말로 삭제 하시겠습니까?')">[삭제하기]</font></a>
		&nbsp; &nbsp; &nbsp;
		</div>
		</td>
...
<?php
////////////// 코맨트 (답글-출력)/////////////
$sql2="SELECT * FROM bbs1_comment where bbs1_no='$bNo' and replys='$d[no]' order by regdate asc"; //asc 오름차순
$result2=$db->query($sql2);
while($d_2=$result2->fetch_array()){
?>
<tr>
...
&nbsp; &nbsp;
		...
		<span style='font-size:8pt; color:#5A5B5A' onclick="return confirm('정말로 삭제 하시겠습니까?')">[삭제하기]</span></a>
		&nbsp; &nbsp; &nbsp;
		</div>
...
<input type=hidden name='bbs1_no' value='<?php echo $bNo;?>' >
...
<hr size='0.1' width='95%' color='#B2B2B2' />
	 ...
	 <tr>
<form name='replys'  action='comment_write.php' method='post'>
<input type=hidden name='bbs1_no' value='<?php echo $bNo?>' title='게시판글 번호'>
<input type=hidden name='replys' value='0'>

// 이름 출력
<td width='150' align='center' valign='middle' bgcolor='#E7CADE'>
<?php
  echo $_SESSION['ses_nickname'];
?>
</td>
<td width='130' align='center' bgcolor='#FFD2F1'>&nbsp;댓글 쓰기</td>
	   <td align='left' bgcolor='#FFD2F1'>
	   <textarea name='memo' cols='80' rows='3' width='' style='width=100%'></textarea>
	   </td>
